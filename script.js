...
function updateCalendar() {
  ...
  days.forEach((day) => {
    ...
    if (relevantOverlaps.length > 1) {
      // Only show breaks/holidays if multiple colleges overlap
      day.classList.add("has-break")

      // Check if all active colleges are overlapping (excluding semester dates from active count for this logic)
      const activeNonSemesterColleges = new Set()
      activeColleges.forEach((collegeId) => {
        const college = academicData[collegeId]
        const hasNonSemesterBreak = college.breaks.some(
          (b) => isDateInRange(date, b.start, b.end) && b.type !== "semester-start" && b.type !== "semester-end",
        )
        if (hasNonSemesterBreak) {
          activeNonSemesterColleges.add(collegeId)
        }
      })

      const activeCollegeCount = activeNonSemesterColleges.size
      const overlappingCollegeCount = relevantOverlaps.length

      if (overlappingCollegeCount === activeCollegeCount && activeCollegeCount > 1) {
        // All toggled colleges (with non-semester breaks) are overlapping - full green
        day.classList.add("full-overlap")
      } else {
        // Some colleges overlapping - crisscross pattern
        day.classList.add("partial-overlap")
      }
    }
